...
if [ "$run_checkm" == "true" ] && [[ ! -s work_files/binsM.stats ]]; then
	...
	for bin_set in $(ls | grep -v tmp | grep -v stats | grep bins); do 
		...
		if [ "$quick" == "true" ]; then
			...
			micromamba run -p /opt/checkm2 checkm2 predict --force --database_path /db/uniref100.KO.1.dmnd -x fa --input $bin_set --output-directory ${bin_set}.checkm2 --threads $threads --lowmem option
		else
			micromamba run -p /opt/checkm2 checkm2 predict --force --database_path /db/uniref100.KO.1.dmnd -x fa --input $bin_set --output-directory ${bin_set}.checkm2 --threads $threads
		fi
		if [[ ! -s ${bin_set}.checkm2/quality_report.tsv ]]; then error "Something went wrong with running CheckM2. Exiting..."; fi
		tsv-utils cut -f 1,2,3,10,12,7,9 ${bin_set}.checkm2/quality_report.tsv \
		|sed "s/$/\t$bin_set/g"                     \
		|tail -n +2                                \
		|tsv-utils add_headline "bin\tcompleteness\tcontamination\tGC\tlineage\tN50\tsize\tbinner" - \
		> ${bin_set}.stats
#		${SOFT}/summarize_checkm.py ${bin_set}.checkm/storage/bin_stats_ext.tsv $bin_set | (read -r; printf "%s\n" "$REPLY"; sort) > ${bin_set}.stats
		...
		rm -r ${bin_set}.checkm2; rm -r ${bin_set}.tmp
...
if [ "$run_checkm" == "true" ] && [ $dereplicate != "false" ]; then
	...
	if [ "$quick" == "true" ]; then
		comm "Note: running with --lowmem option"
		micromamba run -p /opt/checkm2 checkm2 predict --force --database_path /db/uniref100.KO.1.dmnd -x fa --input binsO --output-directory binsO.checkm2 --threads $threads --lowmem option
	else
		micromamba run -p /opt/checkm2 checkm2 predict --force --database_path /db/uniref100.KO.1.dmnd -x fa --input binsO --output-directory binsO.checkm2 --threads $threads
	fi

	if [[ ! -s binsO.checkm2/quality_report.tsv ]]; then error "Something went wrong with running CheckM2. Exiting..."; fi
	rm -r binsO.tmp
	tsv-utils cut -f 1,2,3,10,12,7,9 binsO.checkm2/quality_report.tsv \
	|sed "s/$/\t$bin_set/g"                            \
	|tail -n +2                                        \
	|tsv-utils add_headline "bin\tcompleteness\tcontamination\tGC\tlineage\tN50\tsize\tbinner" - \
	|tsv-utils annotation binsM.stats -                \
	|cut -f 1,2,3,4,5,6,7,15                           \
	> binsO.stats
